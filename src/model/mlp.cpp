...
#include "../parsers/parser_mariposas.cpp"
#include "../parsers/parser_b_m.cpp"
...
public:
    ...
    double predict(field<rowvec> X_test, field<rowvec> Y_test,string label="Test"){
        ...
        cout<<label+" acc: ";
        metrics::print_accuracy(output,Y_matrix);
        metrics::print_matriz_confusion(output,Y_matrix, label);
        return 0;
    }
...
int main(){
    ...
    auto data = Parser_mariposas::get_data(9, 10, 0.8, 0.1, 3, false, false);
    ...
    mlp.fit(X_train,y_train, X_validation, y_validation, 5e-6, 0.2, 25000, "gd", "relu_5_capas_gd_no_augmentation");
