...
class Parser_BM{
    ...
    static double get_category(string filename){
        string line;
        fstream file(filename, ios::in);
        int i = 0;
        getline(file, line);
    ...
    static rowvec one_hot_encode(int category, int total){
        ...
        encoded_label(category) = 1;
        return encoded_label;
    }
public:

    static map<string,field<rowvec>> get_data(float train_proportion=0.8, float validation_proportion=0.1){
        string line, path = "../data/b_m/files/";
        int categories = 2;
        ...
        for(int it=0; it<categories; it++){
            ...
            bool flag = true;
            fstream file("../data/b_m/paths.csv", ios::in);
            file.clear();
